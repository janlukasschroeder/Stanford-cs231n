...
# Source: https://github.com/cthorey/CS231/blob/master/assignment3/cs231n/rnn_layers.py
...
def rnn_step_forward(x, prev_h, Wx, Wh, b):
  ...
  parameter = np.dot(prev_h, Wh) + np.dot(x, Wx) + b
  # parameter = np.dot(Wh, prev_h.T) + np.dot(Wx, x.T) + b

  next_h = np.tanh(parameter)
...
def rnn_step_backward(dnext_h, cache):
  ...
  # calc tanh' = derivative of tanh | IMPORTANT: "* dnext_h" does not belong to derivative
  tan_derivative = (1 - next_h * next_h) * dnext_h

  db = np.sum(tan_derivative, axis=0)
  dx = np.dot(tan_derivative, Wx.T)
  dprev_h = np.dot(tan_derivative, Wh.T)
  dWx = np.dot(x.T, tan_derivative)
  dWh = np.dot(prev_h.T, tan_derivative)
